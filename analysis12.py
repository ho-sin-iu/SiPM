...
Description :
    ...
    * The class "IVsegment" is specially designed for the method ``IVcurve.Vbr1``. 
      It segments a pair of data arrays according to some given range and contains two methods of fit the data:
...
#==============================================================================
class IVcurve():
    def __init__(self, xlabel="Voltage [V]", ylabel="Current [A]", title=None, yscale="linear", grid=True):
        """
        ...
        xlabel : string, optional
            The label for the x-axis. The default is "Voltage [V]".
        ylabel : string, optional
            The label for the y-axis. The default is "Current [A]".
    ...
    def calcPeak(self, order=1, pltMode=0, pltItems=[]):
        """
        Call the method ``IVcurve.Dln`` to evaluate d/dV[ln(I)] or d^2/dV^2[ln(I)]
        and find the peaks on that derivative by ``scipy.signal.find_peaks``.
    ...
    def calcVbr1(self, color=None, pltItems=[]):
        ...
        * (Summary of methods) \F. Nagy *et al.* (2017). *NIMPR, Section A* **849** pp.55-59. https://linkinghub.elsevier.com/retrieve/pii/S0168900217300025
        ...
        BL = IVsegment(vax, iax, vax.min(), vpeak-0.5)  # IVsegment left to vpeak: BL (baseline) ; -0.5 (volts) ---> EMPIRICAL
        WR = IVsegment(vax, iax, vpeak, vax.max()-4)    # IVsegment right to vpeak: WP (working range) ; -4 (volts) ---> EMPIRICAL
        ...
        if "linfit" in pltItems:
            ...
            plt.plot(vfit[_iBmin:_iBmax], iBfit[_iBmin:_iBmax], c=color, ls = ":", lw=1.5, zorder=4, label="") #f"{self.label} (linear)"
        if "polyfit" in pltItems:
            _iWmin, _iWmax = findIndex(vfit, WR.xmin(), WR.xmax()) # indices of the endpoints of WR on vfit
            plt.plot(vfit[_iWmin:_iWmax], best_iWfit[_iWmin:_iWmax], c=color, ls = ":", lw=1.5, zorder=4, label=f"{self.label} (Degree {deg})")
        if "intersect" in pltItems:
            plt.plot(bestvbr, bestivbr, 'ko', markersize=5)
...
class IVsegment():  
    ...
    def polynfit(self, xfit, deg):
        ...
        """
        polyfit = np.polyfit(self.xseg, self.yseg, deg, full=True)
        coef, residuals = polyfit[:2] # polyfit[0] = polynomial coefficients; polyfit[1] = residuals
        yfit = np.polyval(polyfit[0], xfit)
